    ...
    for fmt in ["bestaudio[ext=m4a]/bestaudio", "bestaudio"]:
        try:
            ...
            if url.startswith("http"):
                ...
                return url, duration
            sys.stderr.write(f"yt-dlp bad output {video_id}: stdout={r.stdout[:200]} stderr={r.stderr[:200]}\n")
        except Exception as e:
...
def debug(video_id):
    ...
    try:
        r = sp.run([YTDLP, "--version"], capture_output=True, text=True, timeout=10)
        ytdlp_ver = r.stdout.strip()
    except Exception as e:
        ytdlp_ver = str(e)
    try:
        r = sp.run([YTDLP, "--no-download", "--print", "%(url)s|%(duration)s", "-f", "bestaudio",
                     f"https://www.youtube.com/watch?v={video_id}"],
                    capture_output=True, text=True, timeout=60)
        return jsonify({
        ...
        return jsonify({"error": str(e), "ytdlp_version": ytdlp_ver})
